...
from db import get_db, init_db, dict_from_row, get_api_owner_user_id, OWNER_EMAIL
...
def require_api_key(f):
    ...
    def decorated(*args, **kwargs):
        ...
        api_user_id = get_api_owner_user_id()
        if not api_user_id:
            return jsonify({'error': 'API owner not configured. Register the owner account first.'}), 503
        request.api_user_id = api_user_id
        return f(*args, **kwargs)
    return decorated

def get_current_user_id():
    return getattr(request, 'api_user_id', None) or session.get('user_id')
